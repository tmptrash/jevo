[RpcApi.SimpleOrganism(0x000000000002c460,:(function (o::Creature.Organism,)
        local var_125::Bool = true
        function func_96(var_95::ASCIIString="CMsQ6J5t")
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            var_62 = var_62 + var_62
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::ASCIIString="D42FzkNp",var_44::ASCIIString="QVktoUuw")
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::ASCIIString = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            o.mem[var_13] = var_85
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            if var_26 !== var_26
                Creature.stepDown(o)
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            o.mem[var_13] = var_85
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                Creature.stepRight(o)
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_13] = var_13
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepDown(o)
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            var_85 = var_85 * var_13
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                Creature.stepUp(o)
            end
            if var_16 === var_16
                local var_124::Int16 = 1670
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
                Creature.stepRight(o)
            end
            var_28 = Int(Creature.getEnergy(o,Int(var_13),Int(var_13)))
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_13
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        Creature.stepLeft(o)
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        var_1 = var_1 + var_12
        Creature.stepDown(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.eatRight(o,Int(var_12))
        var_12 = var_12 * var_1
        if var_55 == var_34
            Creature.stepLeft(o)
            Creature.stepRight(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatUp(o,Int(var_1))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_82 !== var_82
        end
        Creature.stepRight(o)
        Creature.stepUp(o)
        var_82 = var_82 / var_82
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        if var_1 != var_1
            local var_122::Bool = false
            Creature.stepUp(o)
        end
        Creature.eatRight(o,Int(var_1))
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        if var_12 <= var_12
            Creature.stepLeft(o)
        end
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_12))
        return true
    end),[100,300,95,0,1,1,1],21,3276,5,772349,0x000001c6,Dict(0=>0,31438=>-29091,-28253=>-28253,-29091=>31438,-2660=>-29091,-5320=>-5320,-572=>-572,1=>1),[41,294],141),RpcApi.SimpleOrganism(0x000000000002c7dc,:(function (o::Creature.Organism,)
        local var_125::Bool = true
        function func_96(var_95::ASCIIString="CMsQ6J5t")
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::ASCIIString="D42FzkNp",var_44::ASCIIString="QVktoUuw")
            Creature.stepRight(o)
            Creature.stepRight(o)
            Creature.stepRight(o)
            Creature.stepDown(o)
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::ASCIIString = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            o.mem[var_13] = var_85
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            if var_26 !== var_26
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            o.mem[var_13] = var_85
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                Creature.stepLeft(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_13] = var_13
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepDown(o)
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            var_85 = var_85 * var_13
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                Creature.stepDown(o)
                Creature.stepUp(o)
            end
            if var_16 === var_16
                local var_124::Int16 = 1670
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
                Creature.stepRight(o)
            end
            var_28 = Int(Creature.getEnergy(o,Int(var_13),Int(var_13)))
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_13
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        Creature.stepLeft(o)
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        var_1 = var_1 + var_12
        Creature.stepDown(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        var_12 = var_12 * var_1
        if var_55 == var_34
            Creature.stepRight(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatUp(o,Int(var_1))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_82 !== var_82
        end
        Creature.stepRight(o)
        Creature.stepUp(o)
        var_82 = var_82 / var_82
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        if var_1 != var_1
            local var_122::Bool = false
            Creature.stepUp(o)
        end
        Creature.eatRight(o,Int(var_1))
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        if var_12 <= var_12
            Creature.stepLeft(o)
        end
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_12))
        return true
    end),[100,300,95,0,1,1,1],21,3276,5,665826,0x000001c3,Dict(0=>0,31438=>-29091,-28253=>-28253,-29091=>31438,-2660=>-29091,-5320=>-5320,-572=>-572,1=>1),[20,287],143),RpcApi.SimpleOrganism(0x000000000002c80d,:(function (o::Creature.Organism,)
        local var_125::Bool = true
        function func_96(var_95::ASCIIString="CMsQ6J5t")
            local var_128::ASCIIString = "JY9lTIP8"
            Creature.stepLeft(o)
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::ASCIIString="D42FzkNp",var_44::ASCIIString="QVktoUuw")
            Creature.stepUp(o)
            Creature.stepRight(o)
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            Creature.stepDown(o)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_131::Bool = true
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::ASCIIString = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            o.mem[var_13] = var_85
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            if var_26 !== var_26
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            o.mem[var_13] = var_85
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                Creature.stepLeft(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_13] = var_13
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepDown(o)
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            var_85 = var_85 * var_13
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                Creature.stepUp(o)
                Creature.stepUp(o)
            end
            if var_16 === var_16
                local var_124::Int16 = 1670
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
                Creature.stepRight(o)
            end
            var_28 = Int(Creature.getEnergy(o,Int(var_13),Int(var_13)))
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_13
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        Creature.stepLeft(o)
        var_55 = var_34 * var_34
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        var_1 = var_1 + var_12
        Creature.stepDown(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        var_12 = var_12 * var_1
        if var_55 == var_34
            Creature.stepRight(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatUp(o,Int(var_1))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_82 !== var_82
        end
        Creature.stepRight(o)
        Creature.stepUp(o)
        var_82 = var_82 / var_82
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        if var_1 != var_1
            local var_122::Bool = false
            Creature.stepUp(o)
        end
        Creature.eatRight(o,Int(var_1))
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        if var_12 <= var_12
            Creature.stepLeft(o)
        end
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepDown(o)
        end
        var_34 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.eatUp(o,Int(var_12))
        return true
    end),[100,300,95,0,1,1,1],2,1354,5,660193,0x000001cb,Dict(0=>0,31438=>-29091,-28253=>-28253,-29091=>31438,-2660=>-29091,-5320=>-5320,-572=>-572,1=>1),[45,292],146),RpcApi.SimpleOrganism(0x000000000002c865,:(function (o::Creature.Organism,)
        local var_125::Bool = true
        function func_96(var_95::ASCIIString="CMsQ6J5t")
            Creature.stepDown(o)
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            Creature.stepDown(o)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::ASCIIString="D42FzkNp",var_44::ASCIIString="QVktoUuw")
            Creature.stepUp(o)
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::ASCIIString = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            o.mem[var_13] = var_85
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            if var_26 !== var_26
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            o.mem[var_13] = var_85
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                Creature.stepLeft(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_13] = var_13
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepDown(o)
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            var_85 = var_85 * var_13
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                Creature.stepUp(o)
            end
            if var_16 === var_16
                local var_124::Int16 = 1670
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
                Creature.stepRight(o)
            end
            var_28 = Int(Creature.getEnergy(o,Int(var_13),Int(var_13)))
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_13
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        Creature.stepLeft(o)
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        var_1 = var_1 + var_12
        Creature.stepDown(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        var_12 = var_12 * var_1
        if var_55 == var_34
            Creature.stepLeft(o)
            Creature.stepRight(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatUp(o,Int(var_1))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_82 !== var_82
        end
        Creature.stepRight(o)
        Creature.stepUp(o)
        var_82 = var_82 / var_82
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        if var_1 != var_1
            local var_122::Bool = false
            Creature.stepUp(o)
        end
        Creature.eatRight(o,Int(var_1))
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        if var_12 <= var_12
            Creature.stepLeft(o)
        end
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_12))
        return true
    end),[100,300,95,0,1,1,1],21,3276,5,587419,0x000001bf,Dict(0=>0,31438=>-29091,-28253=>-28253,-29091=>31438,-2660=>-29091,-5320=>-5320,-572=>-572,1=>1),[26,293],142),RpcApi.SimpleOrganism(0x000000000002c7d1,:(function (o::Creature.Organism,)
        local var_125::Bool = true
        function func_96(var_95::ASCIIString="CMsQ6J5t")
            Creature.stepRight(o)
            Creature.stepUp(o)
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::ASCIIString="D42FzkNp",var_44::ASCIIString="QVktoUuw")
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            Creature.stepUp(o)
            Creature.stepRight(o)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::ASCIIString = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
                Creature.stepUp(o)
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            var_76 = Int(Creature.getEnergy(o,Int(var_85),Int(var_85)))
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            if var_26 !== var_26
                Creature.stepRight(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
            end
            o.mem[var_13] = var_85
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_13] = var_13
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepDown(o)
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            var_85 = var_85 * var_13
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                Creature.stepUp(o)
            end
            if var_16 === var_16
                local var_124::Int16 = 1670
                o.mem[var_124] = var_124
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepUp(o)
                Creature.stepUp(o)
                Creature.stepRight(o)
            end
            var_28 = Int(Creature.getEnergy(o,Int(var_13),Int(var_13)))
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_13
            Creature.stepUp(o)
            return var_8
        end
        local var_1::Int8 = 59
        Creature.stepLeft(o)
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        var_1 = var_1 + var_12
        Creature.stepDown(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_12 = var_12 * var_1
        if var_55 == var_34
            Creature.stepRight(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatUp(o,Int(var_1))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_82 !== var_82
        end
        Creature.stepRight(o)
        Creature.stepUp(o)
        var_82 = var_82 / var_82
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        if var_1 != var_1
            local var_129::Bool = false
            Creature.stepUp(o)
        end
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_12))
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        if var_12 <= var_12
            Creature.stepLeft(o)
        end
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
        end
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_12))
        return true
    end),[100,300,95,0,1,1,1],21,8063,10,578074,0x000001bd,Dict(0=>0,-7978=>31438,31438=>31438,-28253=>-28253,-29091=>31438,-2660=>-29091,1670=>1670,-5320=>-5320,-572=>-572,1=>1),[95,287],141)]































































