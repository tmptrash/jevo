RpcApi.SimpleOrganism(0x0000000000034521,:(function (o::Creature.Organism,)
        local var_161::Int8 = 81
        local var_131::Int16 = 20703
        local var_125::Bool = true
        function func_96(var_95::String="CMsQ6J5t")
            Creature.stepUp(o)
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::String="D42FzkNp",var_44::String="QVktoUuw")
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::String = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
                Creature.stepUp(o)
                Creature.stepUp(o)
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            var_26 = var_26[1:if length(var_26) > length(var_26)
                            0
                        else
                            length(var_26) - length(var_26)
                        end]
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            var_85 = if haskey(o.mem,var_13)
                    o.mem[var_13]
                else
                    var_85
                end
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                local var_169::Int8 = 65
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_85
            Creature.stepDown(o)
            if var_8 !== var_14
                local var_170::String = "jKh2LEpo"
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            Creature.eatRight(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            if var_123 === var_123
                Creature.stepDown(o)
            end
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            var_123 = var_123 + var_76
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                local var_171::Bool = false
                Creature.stepUp(o)
            end
            if var_16 === var_16
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            var_26 = var_26 * var_26
            Creature.eatDown(o,Int(var_14))
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        function func_135(var_134::Int64=6349986163145892178)
            local var_173::Bool = false
            return var_134
        end
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_15 = func_9(var_1)
        Creature.stepDown(o)
        var_131 = if haskey(o.mem,var_131)
                o.mem[var_131]
            else
                var_131
            end
        Creature.stepRight(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        if var_55 == var_34
            Creature.stepLeft(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_34 = var_34 - var_34
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_15))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_1 != var_1
            local var_160::Int64 = 6104843260668152792
            local var_166::Int64 = -7678350092020504724
            Creature.stepRight(o)
            Creature.stepDown(o)
            var_160 = var_160 * var_160
        end
        Creature.stepUp(o)
        var_131 = var_131 * var_82
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        o.mem[var_82] = var_131
        if var_12 <= var_12
            Creature.stepUp(o)
        end
        Creature.stepUp(o)
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.eatDown(o,Int(var_15))
        return true
    end),[100,300,95,0,1,1,1],27,6924,35,1173252,0x00000327,Dict(0=>0,31438=>31438,-28253=>20703,-29091=>31438,-2660=>-29091,20703=>-21217,-5320=>-5320,-572=>-572,1=>1),[141,288],149),RpcApi.SimpleOrganism(0x00000000000342b9,:(function (o::Creature.Organism,)
        local var_131::Int16 = 20703
        local var_125::Bool = true
        function func_96(var_95::String="CMsQ6J5t")
            Creature.stepDown(o)
            Creature.stepRight(o)
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            local var_175::Int64 = 2239460045388253167
            Creature.stepUp(o)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::String="D42FzkNp",var_44::String="QVktoUuw")
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::String = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
                Creature.stepUp(o)
                Creature.stepUp(o)
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            var_26 = var_26[1:if length(var_26) > length(var_26)
                            0
                        else
                            length(var_26) - length(var_26)
                        end]
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            var_28 = Int(Creature.getEnergy(o,Int(var_85),Int(var_13)))
            Creature.eatDown(o,Int(var_14))
            var_13 = var_13 - var_13
            var_85 = if haskey(o.mem,var_13)
                    o.mem[var_13]
                else
                    var_85
                end
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                local var_169::Int8 = 65
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_85
            Creature.stepDown(o)
            if var_8 !== var_14
                local var_170::String = "jKh2LEpo"
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            Creature.eatRight(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            if var_123 === var_123
                Creature.stepDown(o)
            end
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            var_123 = var_123 + var_76
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                local var_171::Bool = false
                Creature.stepUp(o)
                Creature.stepUp(o)
            end
            if var_16 === var_16
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            var_26 = var_26 * var_26
            Creature.eatDown(o,Int(var_14))
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        function func_135(var_134::Int64=6349986163145892178)
            local var_177::Int64 = -5467594387861824762
            local var_173::Bool = false
            local var_174::Int64 = -6626064387828751035
            return var_134
        end
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_15 = func_9(var_1)
        Creature.stepDown(o)
        var_131 = if haskey(o.mem,var_131)
                o.mem[var_131]
            else
                var_131
            end
        Creature.stepRight(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        if var_55 == var_34
            Creature.stepLeft(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_34 = var_34 - var_34
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_15))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_1 != var_1
            local var_160::Int64 = 6104843260668152792
            Creature.stepRight(o)
            Creature.stepDown(o)
            var_160 = var_160 * var_160
        end
        Creature.stepUp(o)
        var_131 = var_131 * var_82
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        o.mem[var_82] = var_131
        if var_12 <= var_12
            Creature.stepUp(o)
        end
        Creature.stepUp(o)
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepLeft(o)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.stepLeft(o)
        return true
    end),[100,300,95,0,1,1,1],27,6924,35,1119017,0x00000335,Dict(0=>0,31438=>31438,-28253=>20703,-29091=>31438,-2660=>-29091,20703=>-27455,-5320=>-5320,-572=>-572,1=>1),[41,260],155),RpcApi.SimpleOrganism(0x000000000003447d,:(function (o::Creature.Organism,)
        local var_161::Int8 = 81
        local var_131::Int16 = 20703
        local var_125::Bool = true
        function func_96(var_95::String="CMsQ6J5t")
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            Creature.stepRight(o)
            Creature.stepDown(o)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::String="D42FzkNp",var_44::String="QVktoUuw")
            local var_173::Int64 = 6699367592251067232
            var_173 = var_173 / var_173
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::String = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
                Creature.stepUp(o)
                Creature.stepUp(o)
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            var_26 = var_26[1:if length(var_26) > length(var_26)
                            0
                        else
                            length(var_26) - length(var_26)
                        end]
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            var_85 = if haskey(o.mem,var_13)
                    o.mem[var_13]
                else
                    var_85
                end
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_85
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            Creature.eatRight(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            if var_123 === var_123
                Creature.stepDown(o)
            end
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            var_123 = var_123 + var_76
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                local var_171::Bool = false
                Creature.stepUp(o)
            end
            if var_16 === var_16
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            var_26 = var_26 * var_26
            Creature.eatDown(o,Int(var_14))
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        function func_135(var_134::Int64=6349986163145892178)
            return var_134
        end
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        Creature.eatDown(o,Int(var_1))
        var_15 = func_9(var_1)
        Creature.stepDown(o)
        var_131 = if haskey(o.mem,var_131)
                o.mem[var_131]
            else
                var_131
            end
        Creature.stepRight(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        if var_55 == var_34
            Creature.stepLeft(o)
            Creature.stepRight(o)
            Creature.stepRight(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_34 = var_34 - var_34
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_15))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_1 != var_1
            local var_160::Int64 = 6104843260668152792
            local var_166::Int64 = -7678350092020504724
            Creature.stepRight(o)
            Creature.stepDown(o)
            var_160 = var_160 * var_160
        end
        Creature.stepUp(o)
        var_131 = var_131 * var_82
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        if var_12 <= var_12
            Creature.stepUp(o)
        end
        Creature.stepUp(o)
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepLeft(o)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.eatDown(o,Int(var_15))
        return true
    end),[100,300,95,0,1,1,1],27,6924,84,1011125,0x00000333,Dict(0=>0,31438=>31438,-28253=>20703,-29091=>31438,-2660=>-29091,20703=>20703,-5320=>-5320,-572=>-572,1=>1),[146,289],152),RpcApi.SimpleOrganism(0x0000000000034497,:(function (o::Creature.Organism,)
        local var_161::Int8 = 81
        local var_131::Int16 = 20703
        local var_125::Bool = true
        function func_96(var_95::String="CMsQ6J5t")
            var_95 = var_95[1:if length(var_95) > length(var_95)
                            0
                        else
                            length(var_95) - length(var_95)
                        end]
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            Creature.stepDown(o)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::String="D42FzkNp",var_44::String="QVktoUuw")
            local var_172::Int64 = 7998535106955226490
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::String = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
                Creature.stepUp(o)
                Creature.stepUp(o)
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            Creature.stepLeft(o)
            var_26 = var_26[1:if length(var_26) > length(var_26)
                            0
                        else
                            length(var_26) - length(var_26)
                        end]
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_8))
            var_13 = var_13 - var_13
            var_85 = if haskey(o.mem,var_13)
                    o.mem[var_13]
                else
                    var_85
                end
            Creature.eatDown(o,Int(var_14))
            if var_28 !== var_28
                local var_169::Int8 = 65
                Creature.stepUp(o)
            end
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_76 = var_76 * var_76
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_85
            Creature.stepDown(o)
            if var_8 !== var_14
                local var_170::String = "jKh2LEpo"
                Creature.stepUp(o)
            end
            Creature.eatRight(o,Int(var_8))
            if var_26 === var_26
                Creature.stepDown(o)
                Creature.stepDown(o)
            end
            Creature.eatRight(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            if var_123 === var_123
                Creature.stepDown(o)
            end
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            Creature.eatDown(o,Int(var_14))
            var_123 = var_123 + var_76
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                local var_171::Bool = false
                Creature.stepUp(o)
            end
            if var_16 === var_16
                Creature.stepUp(o)
                Creature.stepRight(o)
                Creature.stepRight(o)
            end
            var_26 = var_26 * var_26
            Creature.eatDown(o,Int(var_14))
            Creature.stepUp(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_8))
            return var_8
        end
        local var_1::Int8 = 59
        function func_135(var_134::Int64=6349986163145892178)
            return var_134
        end
        var_12 = func_9(var_1)
        var_12 = func_9(var_12)
        var_15 = func_9(var_12)
        var_1 = func_9(var_1)
        var_15 = func_9(var_1)
        Creature.stepDown(o)
        var_131 = if haskey(o.mem,var_131)
                o.mem[var_131]
            else
                var_131
            end
        Creature.stepRight(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        if var_55 == var_34
            Creature.stepLeft(o)
        end
        var_12 = func_9(var_12)
        Creature.stepUp(o)
        Creature.eatRight(o,Int(var_12))
        o.mem[var_82] = var_82
        var_34 = var_34 - var_34
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.eatUp(o,Int(var_15))
        Creature.stepUp(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        Creature.stepUp(o)
        if var_1 != var_1
            local var_160::Int64 = 6104843260668152792
            local var_166::Int64 = -7678350092020504724
            Creature.stepDown(o)
            Creature.stepRight(o)
            Creature.stepDown(o)
            var_160 = var_160 * var_160
        end
        Creature.stepUp(o)
        var_131 = var_131 * var_82
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        o.mem[var_82] = var_131
        if var_12 <= var_12
            Creature.stepUp(o)
        end
        Creature.stepUp(o)
        for i::Int8 = 1:div(var_1,_LOOP_STEPS_DIVIDER)
            Creature.stepDown(o)
        end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.eatDown(o,Int(var_15))
        return true
    end),[100,300,95,0,1,1,1],27,6924,35,936169,0x00000322,Dict(0=>0,31438=>31438,-28253=>20703,-29091=>31438,-2660=>-29091,20703=>-6463,-5320=>-5320,-572=>-572,1=>1),[116,289],151),RpcApi.SimpleOrganism(0x00000000000343fb,:(function (o::Creature.Organism,)
        function func_96(var_95::String="CMsQ6J5t")
            return var_95
        end
        local var_82::Int16 = -28253
        function func_63(var_62::Int64=-5487471262987408691)
            return var_62
        end
        local var_55::Int64 = 8062249648076403993
        function func_45(var_43::String="D42FzkNp",var_44::String="QVktoUuw")
            Creature.stepRight(o)
            Creature.stepLeft(o)
            Creature.stepUp(o)
            return var_43
        end
        local var_34::Int64 = -6982312523886405890
        function func_25(var_24::Int64=5926471544133193903)
            return var_24
        end
        function func_9(var_8::Int8=-18)
            local var_139::Int64 = -4013247465467956436
            local var_137::Bool = true
            local var_131::Int8 = -38
            local var_123::Int64 = 2850089338817388517
            local var_85::Int16 = -29091
            local var_76::Int64 = -8364194871863581978
            local var_57::Bool = true
            local var_28::Int64 = -5547387289894786475
            local var_26::String = "zSGt1MST"
            local var_16::Bool = true
            local var_14::Int8 = 115
            local var_13::Int16 = 31438
            Creature.stepLeft(o)
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            Creature.stepUp(o)
            Creature.eatLeft(o,Int(var_14))
            var_131 = var_14 * var_8
            Creature.stepUp(o)
            o.mem[var_85] = var_13
            if var_28 > var_76
            end
            o.mem[var_13] = var_13
            Creature.eatUp(o,Int(var_8))
            Creature.eatLeft(o,Int(var_8))
            var_26 = var_26 * var_26
            Creature.eatLeft(o,Int(var_14))
            var_13 = if haskey(o.mem,var_13)
                    o.mem[var_13]
                else
                    var_13
                end
            Creature.eatLeft(o,Int(var_8))
            Creature.stepLeft(o)
            var_26 = var_26 * var_26
            Creature.stepLeft(o)
            Creature.stepDown(o)
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_85
            var_76 = Int(Creature.getEnergy(o,Int(var_85),Int(var_85)))
            Creature.eatLeft(o,Int(var_8))
            Creature.stepDown(o)
            if var_26 == var_26
            end
            if var_26 !== var_26
                local var_138::Int64 = 4138323514544043370
                var_138 = var_138 * var_138
                var_138 = var_138 - var_138
                Creature.stepRight(o)
            end
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_8))
            Creature.eatDown(o,Int(var_14))
            var_13 = var_13 - var_13
            Creature.eatRight(o,Int(var_14))
            Creature.stepDown(o)
            Creature.eatLeft(o,Int(var_14))
            Creature.stepUp(o)
            Creature.eatDown(o,Int(var_14))
            Creature.stepLeft(o)
            var_16 = var_137 | var_16
            Creature.eatDown(o,Int(var_14))
            o.mem[var_13] = var_13
            Creature.stepDown(o)
            if var_8 !== var_14
                Creature.stepUp(o)
            end
            var_57 = var_16 & var_137
            Creature.eatRight(o,Int(var_8))
            Creature.stepUp(o)
            var_85 = var_85 * var_13
            Creature.stepDown(o)
            Creature.eatUp(o,Int(var_8))
            if var_14 !== var_14
                Creature.stepUp(o)
            end
            if var_16 === var_16
                local var_124::Int16 = 1670
                Creature.stepRight(o)
                var_124 = var_124 / var_124
                Creature.stepUp(o)
                Creature.stepRight(o)
            end
            Creature.eatDown(o,Int(var_14))
            o.mem[var_85] = var_13
            Creature.stepUp(o)
            Creature.eatRight(o,Int(var_14))
            return var_8
        end
        local var_1::Int8 = 59
        var_12 = func_9(var_1)
        var_1 = func_9(var_12)
        var_1 = func_9(var_1)
        var_1 = var_1 + var_12
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.stepRight(o)
        var_1 = var_1 / var_12
        if var_55 == var_34
            Creature.stepDown(o)
        end
        var_12 = func_9(var_12)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatRight(o,Int(var_12))
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.eatUp(o,Int(var_1))
        Creature.stepRight(o)
        Creature.eatDown(o,Int(var_1))
        Creature.stepUp(o)
        Creature.stepRight(o)
        Creature.stepUp(o)
        var_34 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.eatLeft(o,Int(var_12))
        Creature.stepRight(o)
        Creature.stepDown(o)
        Creature.eatLeft(o,Int(var_1))
        Creature.stepRight(o)
        Creature.stepUp(o)
        Creature.stepUp(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepRight(o)
        var_55 = Int(Creature.getEnergy(o,Int(var_82),Int(var_82)))
        Creature.eatRight(o,Int(var_12))
        Creature.eatRight(o,Int(var_1))
        Creature.stepRight(o)
        Creature.stepRight(o)
        var_82 = if haskey(o.mem,var_82)
                o.mem[var_82]
            else
                var_82
            end
        Creature.stepUp(o)
        Creature.eatLeft(o,Int(var_12))
        Creature.stepUp(o)
        if var_55 <= var_34
            Creature.stepDown(o)
            Creature.stepDown(o)
            Creature.stepDown(o)
        end
        Creature.stepDown(o)
        Creature.stepDown(o)
        Creature.stepRight(o)
        return true
    end),[100,300,95,0,1,1,1],23,8078,45,932620,0x000002ef,Dict(0=>0,31438=>31438,-28253=>-28253,-29091=>-29091,-2660=>-29091,-5320=>-5320,-572=>-572,1=>1),[22,4],134)]