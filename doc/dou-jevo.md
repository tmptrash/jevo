...
Следует так же упомянуть, что все начиналось с пустого кода, а если быть точным, то с пустой функции. А пример выше, сформировался в результате нескольких десятков тысяч поколений. Что он делает? Эти функции `Creature.eatUp()` и `Creature.eatDown()` означают "укуси" сверху или снизу относительно собственной позиции. Последний параметр, `var_1` - это то, сколько "откусить". По сути, он просто сидит и ждет пока к нему кто-то подползет, после чего, жертва будет съедена. Вы спросите: "Откуда организм знает, какие функции нужно вызывать?" или "Какие функции вообще существуют?". Систама предоставляет некий набор из:
