...
- Во время написания этой статьи появился первый организм, который научился выполнять примитивные логические действия! Для меня это был настоящий праздник! :) Давайте взглялем на ту часть его кода, которая выполняет условную конструкцию:
```julia
    ...
    ...
    var_2 = if 127 >= abs(var_33)         # если "под нами" энергия или другой организм, то var_2 = 127 иначе var_2 = 0
            Int8(var_33)                  # var_2 = 0
        else
            127                           # var_2 = 127
        end
    for i::Int8 = 1:div(var_2,16)         # если var_2 !== 0 (то есть, под нами кто-то есть) то крутимся здесь 7 раз и беспощадно кусаем
        ...
        Creature.eatDown(c,o,Int(var_36)) # откусываем 127 "энергитических юнитов" от соседа снизу
    end
    ...
```
